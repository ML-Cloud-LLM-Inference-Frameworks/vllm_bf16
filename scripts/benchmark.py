...
async def run_one_concurrency(
    ...
    async def one_request(row):
        ...
        async with sem:
            ...
            # capture ttft_s from server response when available (HF baseline returns it)
...
async def main():
    ...
    parser.add_argument(
        "--nvidia-smi-csv",
        default=None,
        help="Log GPU stats via nvidia-smi during benchmark; sweep: _cN suffix added automatically",
    )
